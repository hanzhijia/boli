...
with st.container():
    ...
    with col_p1:    与列_p1：
        st.markdown("**1. 废料输入**")        st.标记("**1. 废料输入**")
    ...
    with col_p2:    与列_p2：
        ...
        fridge_magnet_count = int((raw_weight * 1000 * 0.6) / 25) # 60% 做冰箱贴        冰箱贴数量 = 整数((原料重量 * 1000 * 0.6) / 25) # 60% 用于制作冰箱贴
        keychain_count = int((raw_weight * 1000 * 0.4) / 15)      # 40% 做钥匙扣        钥匙扣数量 = int((原料重量 * 1000 * 0.4) / 15)      # 40% 制作钥匙扣
        
        st.metric(label="可制作环保冰箱贴", value=f"{fridge_magnet_count} 个")
        st.metric(label="可制作艺术钥匙扣", value=f"{keychain_count} 个")
        
    with col_p3:    使用 col_p3:
        st.markdown("**3. 收益测算**")
        # 市场定价假设：冰箱贴 15元/个，钥匙扣 9.9元/个
        revenue = fridge_magnet_count * 15 + keychain_count * 9.9
        # 扣除辅料成本 (UV胶、底托等约 20%)
        net_profit = revenue * 0.8        净利润 = 收入 * 0.8        净利润 = 收入 * 0.8        净利润 = 收入 * 0.8
...
with col_road_right:
    ...
    <div class="roadmap-container">
        ...
        <div class="roadmap-item">
            <div class="roadmap-date">第二阶段：社会服务与商业化扩张 (进行中)</div>            <div class="roadmap-date">第二阶段：社会服务与商业化扩张（进行中）</div>
            <div class="roadmap-content">
    ...
    ''', unsafe_allow_html=True)
with col_road_left:带有col_road_left：带有col_road_left：带有col_road_left： col_road_left：带有col_road_left：带有col_road_left：带有col_road_left：
    # 留白或放个插图
